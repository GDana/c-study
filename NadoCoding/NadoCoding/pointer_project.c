...
// 사막이 너무 더워서.. 너무 건조해서 물이 아주 빨리 증발 해요.
...
int main(void) {
	...
	long prevElapesdTime = 0;	//직전 경과 시간(최근에 물을 준 시간 간격)
	...
	while (1) {
		...
		// 어항의 물을 감소 (증발)
		decreaseWater(prevElapesdTime);

		// 사용자가 입력한 어항에 물을 준다
		// 1. 어항의 물이 0 이면? 물을 주지 않는다.. 이미 고기가.. 쥭..엇..
		if(cursor[num - 1] <= 0){
		...
		prevElapesdTime = totalElapsedTime;
